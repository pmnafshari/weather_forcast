...
function getAvgRainNext6h(hourly: HourlyData[]): number {
  ...
  const next6 = hourly
    .filter(h => {
      ...
      return t >= now && t <= new Date(now.getTime() + 6 * 3_600_000);
    })
...
function findBestTime(hourly: HourlyData[], config: ActivityConfig): {
  best: string | null;
  avoid: string | null;
  ...
  return {
    best: bestStart >= 0 ? fmt(bestStart) : null,
    avoid: worstStart >= 0 && worstScore < 50 ? fmt(worstStart) : null,
    avoidReason,
  };
}
