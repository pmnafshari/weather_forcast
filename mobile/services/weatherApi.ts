...
const OW_API_KEY = process.env.EXPO_PUBLIC_OPENWEATHER_API_KEY ?? '';
  ...
  for (let i = 0; i <= retries; i++) {
    try {
      ...
      if (res.status === 429 && i < retries) {
        console.warn('[weatherApi] Rate limited, retrying...');
        await new Promise(r => setTimeout(r, delay * (i + 1)));
        continue;
      }
      return res;
    } catch (e) {
      console.error(`[weatherApi] Fetch failed (attempt ${i + 1}/${retries + 1}):`, e);
      if (i < retries) {
        await new Promise(r => setTimeout(r, delay * (i + 1)));
        continue;
      }
...
export async function fetchWeatherData(lat: number, lon: number): Promise<WeatherData> {
  ...
  if (!currentRes.ok) {
    ...
    console.error('[weatherApi] Weather API error:', currentRes.status, body);
    throw new Error(`Weather API error: ${currentRes.status} – ${body || 'Unknown error'}`);
  }
  if (!forecastRes.ok) {
    const body = await forecastRes.text().catch(() => '');
    console.error('[weatherApi] Forecast API error:', forecastRes.status, body);
    throw new Error(`Forecast API error: ${forecastRes.status} – ${body || 'Unknown error'}`);
  }
...
function mapOpenWeatherResponse(current: OWCurrent, forecast: OWForecastResponse): WeatherData {
  ...
  // Build hourly from forecast (3-hour intervals), prepend current as "now"
